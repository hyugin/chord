...
CASK lives quietly in your menu bar and answers one question fast: _"What can I press right now?"_

Click the menubar icon and CASK reads the frontmost app, then shows the custom Karabiner shortcuts you've actually defined for it — pulled live from your `karabiner.json` and labelled from each rule's description. Need the full picture? One click opens a complete cheat sheet of every shortcut available for the current app, KeyClu-style.
...
1. CASK watches which application is frontmost.
...
- **Open from the menubar** — click CASK → **Open Keyboard Map**.
...
Or open the package in Xcode via **File → Open** and select `Package.swift`, then build and run the **CASK** scheme (⌘R).

Building from source requires Xcode (or the Swift toolchain).

### Install from a release DMG

Pre-built DMGs are published on [GitHub Releases](https://github.com/<you>/CASK/releases). CASK is ad-hoc signed (no Apple Developer account), so macOS Gatekeeper may block the first launch.

1. Download `cask-<version>.dmg` and `cask-<version>.dmg.sha256` from the latest release.
2. Verify the download was not altered:

```bash
shasum -a 256 -c cask-<version>.dmg.sha256
```

You should see `cask-<version>.dmg: OK`. The release notes also list the SHA-256 hash if you prefer to compare manually.

3. Open the DMG and drag **CASK** to **Applications**.
4. On first launch, macOS may say the developer cannot be verified. **Right-click CASK → Open**, then confirm. You only need to do this once.
...
1. Launch CASK — it appears in your menu bar.
2. Switch to any app, then click the CASK icon.
...
CASK reads from the standard Karabiner config location:

```
~/.config/karabiner/karabiner.json
```

For the cleanest labels, give each `complex_modifications` rule a clear `description` — that's what CASK displays. App-specific rules (scoped via `frontmost_application_if`) show under the matching app; everything else shows in your global layer.

## Roadmap

- [ ] Search/filter within the current app's shortcuts
- [ ] Global hotkey to open CASK without reaching for the mouse
...
CASK is inspired by KeyClu — the same glanceable, context-aware spirit, focused on surfacing your custom Karabiner layer.
